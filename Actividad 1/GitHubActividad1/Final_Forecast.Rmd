...
#1.Demand Models for Internet  Residential Subscribers
...
plot(tssubs1,main="Internet Residencial Mbps",sub="Clientes 6 Mbps")
...
##Sets of predictions and forecasting for 5 years are:
````{r}
...
#2.Demand Models for Internet Bussines Subscribers
...
###Forecasting for 5 years
````{r}
fore5a<-data.frame(Fore_Log1=alog[1:5],Fore_Gom1=agom[1:5],Fore_Par1=para)
fore5b<-data.frame(Fore_Log2=blog[1:5],Fore_Gom2=bgom[1:5],Fore_Par2=parb)
fore5c<-data.frame(Fore_Log3=clog[1:5],Fore_Gom3=cgom[1:5],Fore_Par3=parc)
````
...
#4.Demand Models for VoIP Subscribers
...
###Forecasting for 5 years VoIP
````{r}
fore7a<-data.frame(Fore_Log1=alog[1:5],Fore_Gom1=agom[1:5],Fore_Par1=para)
fore7b<-data.frame(Fore_Log2=blog[1:5],Fore_Gom2=bgom[1:5],Fore_Par2=parb)
fore7c<-data.frame(Fore_Log3=clog[1:5],Fore_Gom3=cgom[1:5],Fore_Par3=parc)
````


##4.3 Predicting with Fisher Pry
Model applied When substitution is driven by superior technology. 
The new product or service presents some technological advantage over the old one.
...
##4.5 Performance Models for VoIP
Using the forecast package, the performance models are:
**(ME)**:Mean Error 
**(RMSE)**:Root Mean Square Error
**(MAE)**: Mean Absolute Error
...
#5.Predictions, Forecast and Performance Results

`````{r echo=TRUE}
library(knitr)
````
##5.1. Predictions Report
The data set for fit each model are:
...
kable(predIRes,format = "markdown",caption ="Internet Residential")
kable(predIBus,format = "markdown",caption = "Internet Bussines")
kable(predVoD,format = "markdown",caption = "VoD")
kable(predVoIP,format = "markdown",caption = "VoIP")
````


````{r,echo=FALSE}
#predictall<-data.frame(Res=predIRes,VoD=predVoD,VoIP=predVoIP,Bus=predIBus)
#write.csv2(predictall,"Predictions_Report.csv")
````

##5.2. Forecast Report
The forecast by model are:
...
kable(foreIRes1,format = "markdown",caption = "Internet Residential - OP1")
kable(foreIRes2,format = "markdown",caption = "Internet Residential - OP2")
kable(foreIRes3,format = "markdown",caption = "Internet Residential - OP3")

kable(foreIBus1,format = "markdown",caption = "Internet Bussines - OP1")
kable(foreIBus2,format = "markdown",caption = "Internet Bussines - OP2")
kable(foreIBus3,format = "markdown",caption = "Internet Bussines - OP3")

kable(foreVoD1,format = "markdown",caption = "VoD - OP1")
kable(foreVoD2,format = "markdown",caption = "VoD - OP2")
kable(foreVoD3,format = "markdown",caption = "VoD - OP3")

kable(foreVoIP1,format = "markdown",caption = "VoIP - OP1")
kable(foreVoIP2,format = "markdown",caption = "VoIP - OP2")
kable(foreVoIP3,format = "markdown",caption = "VoIP - OP3")
````


````{r,echo=FALSE}

#forecastall<-data.frame(Res=foreIRes,VoD=foreVoD,VoIP=foreVoIP,Bus=foreIBus)
#write.csv2(forecastall,"Forecasting_Report.csv")
````

##Performance Report
````{r, echo=TRUE}
#Internet Residential
performIRes<-data.frame(performIRes1,performIRes2[2:6],row.names=NULL)
pres3<-data.frame(performIRes3,row.names=NULL)
#Internet Bussines
performIBus<-data.frame(performIBus1,performIBus2[2:6],row.names=NULL)
pbus3<-data.frame(performIBus3,row.names=NULL) 
#Video on Demand
performVoD<-data.frame(performVoD1,performVoD2[2:6],row.names=NULL)
pvod3<-data.frame(performVoD3,row.names=NULL)     
#Voice over IP
performVoIP<-data.frame(performVoIP1,performVoIP2[2:6],row.names=NULL)
pvoip3<-data.frame(performVoIP3,row.names=NULL) 
````

Internet Residential
````{r, echo=FALSE}
kable(performIRes,format = "markdown")
kable(pres3,format = "markdown")
````

Ìnternet Bussines
````{r, echo=FALSE}
kable(performIBus,format = "markdown")
kable(pbus3,format = "markdown")
````

Video on Demand
````{r, echo=FALSE}
kable(performVoD,format = "markdown")
kable(pvod3,format = "markdown")
````

Voice over IP
````{r, echo=FALSE}
kable(performVoIP,format = "markdown")
kable(pvoip3,format = "markdown")
````

````{r, echo=FALSE}
#performall<-data.frame(Res=performIRes,VoD=performVoD,VoIP=performVoIP)#,performIBus)
